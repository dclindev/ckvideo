...
class HostChecker:
    ...
    def _check_single_host(self, host: str) -> bool:
        ...
        html, final_host = self.client.get_page(host)
        if final_host != host:
            logger.info(f"主机跳转到: {final_host}")
        result = self._extract_host_info(html, final_host)

        if result is True:
            self.config.save_host(final_host)
            logger.info(f"找到有效主机: {final_host}")
            return True
